...
public class Graphviz {
    ...
    public void createDemoFromDot(String Location, String Storage) throws IOException{
        File file = new File(Location);
        FileInputStream fis = null;
        fis = new FileInputStream(file);

        MutableGraph g = Parser.read(fis);
        guru.nidi.graphviz.engine.Graphviz.fromGraph(g).width(700).render(Format.PNG).toFile(new File(Storage + "\\example"+ cont +".png"));
        cont++;
    }
}
